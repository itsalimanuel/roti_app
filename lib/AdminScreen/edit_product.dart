...
import 'package:roti_app/AdminScreen/add_categoreis.dart';
...
class _AddProductsState extends State<EditProducts> {
  ...
  var progress1 = new ProgressDialog(
    backgroundColor: Colors.black12,
    color: Colors.white,
    containerColor: MyApp.appColors,
    borderRadius: 5.0,
  ...
  Future _getCategories() async {
    ...
    reference.once().then((snapshot) {
      ...
      for (int s = 0; s < val.keys.length; s++) {
        ...
        _productCategories.add(val2[categoryName].toString());
      }
      setState(() {
        Navigator.pop(context);
      });
    ...
    for (int s = 0; s < fbImages.length; s++) {
      if (!fbImages[s].toString().contains("https")) {
        StorageReference storageReference = FirebaseStorage.instance
            ...
            .child(refKey)
            .child(refKey + "$s.jpg");
         StorageUploadTask uploadTask = storageReference.putFile(new File(fbImages[s]));
